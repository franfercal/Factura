...
class UserChooseProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            grupos = Group.objects.filter(id=self.kwargs['pk'])
            if not grupos.exists():
                request.session['group'] = None
            else:
                grupo_elegido = grupos[0]
                # Mismo formato que en User.set_group_session (compatible con JSONSerializer).
                request.session['group'] = {
                    'id': grupo_elegido.id,
                    'name': grupo_elegido.name,
                }
        except Exception:
            pass
